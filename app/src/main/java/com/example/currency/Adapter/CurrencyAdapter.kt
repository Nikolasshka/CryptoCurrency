...
//слушатель клика по элементам списка
        ...
        override fun bind(item: Any) {
            let {
                ...
                view.tvPriceChangePercentage24h.text = item.priceChangePercentage24h.toString()
                if(item.priceChangePercentage24h > 0)
                    view.tvPriceChangePercentage24h.setTextColor(Color.GREEN)
                else view.tvPriceChangePercentage24h.setTextColor(Color.RED)
                id = item.id
                symbol = item.symbol
                name = item.name
                image = item.image
